...
```python
cube = myhdl.Array( (3, 3, 3), Signal( intbv(0)[WIDTH_D:]) )
```
The tuple _(3, 3, 3)_ defines the _shape_ of the multidimensional array, and _Signal( intbv(0)[WIDTH_D:])_ defines the _dtype_.  
...
begin
	...
	begin
		...
		for l in 0 to 3 - 1 loop
			for k in 0 to 3 - 1 loop
				for j in 0 to 3 - 1 loop
					cubesum := (cubesum + to_unsigned(cube(l)(k)(j), 1));
				end loop;
			end loop;
		end loop;
		Q <= cubesum;
...
```python
def nDmatrix( Clk, Reset, A0, A1, D , Wr1, Wr2 , Q , Shift):
    ''' a small example to test the nD experimental implementation '''
    ...
    def fill():
            ...
            if Shift:
                for k in range(3):
                    matrix[k][0:2].next = matrix[k][1:3]
...
architecture MyHDL of nDmatrix is
	...
	type t_array_matrix_0 is array (0 to 3 - 1) of unsigned(7 downto 0);
	type t_array_matrix is array (0 to 3 - 1) of t_array_matrix_0;
...
begin
	...
	begin
		if (Reset = '1') then
			...
			matrix(0)(0) <= to_unsigned(0, 8);
			matrix(0)(1) <= to_unsigned(0, 8);
			matrix(0)(2) <= to_unsigned(0, 8);
			matrix(1)(0) <= to_unsigned(0, 8);
			matrix(1)(1) <= to_unsigned(0, 8);
			matrix(1)(2) <= to_unsigned(0, 8);
			matrix(2)(0) <= to_unsigned(0, 8);
			matrix(2)(1) <= to_unsigned(0, 8);
			matrix(2)(2) <= to_unsigned(0, 8);
		elsif rising_edge(Clk) then
			for k in 0 to 3 - 1 loop
				for j in 0 to 3 - 1 loop
					if ((signed(resize(A1, 3)) = k) and (signed(resize(A0, 3)) = j) and bool(Wr1)) then
						matrix(k)(j) <= D(0);
					end if;
				end loop;
			end loop;
			for j in 0 to 3 - 1 loop
				if ((signed(resize(A0, 3)) = j) and bool(Wr2)) then
					vector(j) <= D(0);
				end if;
			end loop;
			if bool(Shift) then
				for k in 0 to 3 - 1 loop
					matrix(k)(2 - 1 downto 0) <= matrix(k)(3 - 1 downto 1);
					matrix(k)(2)              <= vector(k);
				end loop;
			end if;
		end if;
	...
	begin
		...
		for l in 0 to 3 - 1 loop
			for k in 0 to 3 - 1 loop
				for j in 0 to 3 - 1 loop
					matrixsum := (matrixsum + to_unsigned(matrix(l)(k)(j), 1));
				end loop;
			end loop;
		end loop;
		Q <= matrixsum;
...
always @(posedge Clk, posedge Reset) begin: nDmatrix_fill
    ...
    if (Reset == 1) begin
        ...
        for (k=0; k<3; k=k+1) begin
            for (j=0; j<3; j=j+1) begin
                if ((($signed({1'b0, A1}) == k) && ($signed({1'b0, A0}) == j) && Wr1)) begin
                    matrix[k][j] <= D;
                end
            end
        end
        for (j=0; j<3; j=j+1) begin
            if ((($signed({1'b0, A0}) == j) && Wr2)) begin
                vector[j] <= D;
            end
        end
        if (Shift) begin
...
Perhaps it is time to move on to genrating _SystemVerilog_ output?
